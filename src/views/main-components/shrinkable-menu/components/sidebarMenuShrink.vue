                ...
                <Dropdown transfer v-if="item.children.length !== 1" placement="right-start" :key="index" @on-click="changeMenu">
                    ...
                    <template v-for="(child, i) in item.children">
                        <DropdownMenu style="width: 200px;" slot="list" >
                            <template  v-if="!child.hasOwnProperty('children')">
                            ...
                            <template  v-else>
                                <Dropdown placement="right-start" transfer>
                                    <DropdownItem :name="child.name" :key="i"><Icon :type="child.icon"></Icon><span style="padding-left:10px;">{{ itemTitle(child) }}</span></DropdownItem>
                                    <DropdownMenu style="padding-left:70px;width: 200px;" slot="list">
                                        <template v-for="(threeChild, i) in child.children">
                                            <DropdownItem :name="threeChild.name" :key="i"><Icon :type="threeChild.icon"></Icon><span style="padding-left:10px;">{{ itemTitle(threeChild) }}</span></DropdownItem>
                                        </template>
                                    </DropdownMenu>
                                </Dropdown>
                            </template>
